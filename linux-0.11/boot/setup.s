...
! ok，整个读磁盘过程都正常，现在将光标位置保存以备今后使用
	mov	ax,#INITSEG	! this is done in bootsect already, but...
      ...
      ! 设置过，但是现在是 setup 程序，Linus 觉得需要再重新设置一下。
	mov	ds,ax
...
do_move:
	mov	es,ax		! destination segment  ! es:di赋值目的地址(初始为 0x0000:0x0)
	add	ax,#0x1000
	cmp	ax,#0x9000  ! 已经把从 0x8000 段开始的 64k 代码移动完？
	jz	end_move
	mov	ds,ax		! source segment  ！di赋值源地址(初始为 0x1000:0x0)
	sub	di,di
	sub	si,si
...
!! 我们将它们放在正好处于 intel 保留的硬件中断后面， 在 int 0x20-0x2F。
	mov	al,#0x11		! initialization sequence ! 0x11 表示初始化命令开始，是 ICW1 命令字，表示边
